...
import { FormattedMessage, useIntl, useRouteData } from '@umijs/max';
...
import {
    ...
    PostManageControllerValue_4,
    PostManageControllerValue_3,
...
const Add = (props: any) => {
    ...
    const handleAdd = async (fields: any) => {
        ...
        try {
            await PostManageControllerValue_3({  ...fields,
                id:articleId });
            hide();
    ...
    const [editInitValues, setEditInitValues] = useState<any>();
    const CustomButton = () => <span className="">button1</span>;
    const articleId = history.location.search.slice(4, history.location.search.length);
    ...
    useEffect(()=>{
        editForm.setFieldsValue({...editInitValues})
    },[editInitValues])
    const [cardBind,setCardBind] = useState<any []>();
    useEffect(()=>{
        console.log(cardBind,"cardBind")
    },[cardBind])
    useEffect(() => {
        ...
        PostManageControllerValue_4(articleId).then((res) => {
            if(!res)return
            console.log('success',res)
            // set
            setDescription(res.body);
            setDescriptionEn(res.body_en);
            // setTags(res.keywords);
            setImageUrl(`${SITE_URL}/api/medias/images/${res.background.id}${res.background.ext}`);
            setImageId(res.background.id);
            setThumbId(res.thumb.id)
            setThumbUrl(`${SITE_URL}/api/medias/images/${res.thumb.id}${res.thumb.ext}`);
            setEditInitValues({
                title: res.title,
            ...
            setTags(res.keywords);
            setTags2(res.tags);
            setCardBind(res.cards.slice().filter((item: any) => item.level !== 2)
            .map((item: any) => {
    ...
    // if(!editInitValues) return
    return (
        <ProForm
            // enableReinitialize={true}
            // request={async () => {
            //     const result = await  PostManageControllerValue_4(articleId)
            //     return result;
            // }}
            form={editForm}
        ...
        >
            ...
            <ProFormGroup title="侧边栏卡片（多选）">
                <ProFormSelect
                    ...
                    // value={cardBind}
                    // defaultValue={cardBind}
                    mode="multiple"
                    // tagRender={(e)=>{
                    //     console.log(e,"tagRender")
                    // }}
                    width={750}
                    name="cards"
                />
